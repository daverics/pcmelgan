...
def load_models(mel2wav_path, device=get_default_device()):
    """
    Args:
        mel2wav_path (str or Path): path to the root folder of dumped text2mel
        device (str or torch.device): device to load the model
    ...
    model_f = UNetFilter(1, 1, chs=[ 2 ,4, 8, 16,32], image_width=32, image_height=80, noise_dim=10, nb_classes=2, embedding_dim=16).to(device)
    model_f.load_state_dict(torch.load(root/"model_f.pt"))
    netG.load_state_dict(torch.load(root / "best_netG.pt", map_location=device))
